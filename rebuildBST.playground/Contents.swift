...
class Solution {
    ...
    func rebuildBST(_ root: TreeNode?) {
        guard root != nil else { return }
        var flatten = bfs(root).sorted()
        let newRoot = TreeNode(flatten[Int((flatten.count-1)/2)])
        flatten.remove(at: Int((flatten.count-1)/2))

        for n in flatten{
            insertToTree(TreeNode(n), newRoot)
        }
...
let tree = solution.createTreeFromLevelOrder([1,3,nil,nil,2])
print("From: \n\n",tree!.asString, "\n\nTo:\n")
solution.rebuildBST(tree)
print(tree!.asString)
