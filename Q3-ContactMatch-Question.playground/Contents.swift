...
// Clarification question: does the content sort based on name? assume it's No.


public func solution(_ A : [String], _ B : [String], _ P : String) -> String {
    // O(n)
    var indexPointer = 0
    var foundMatch:[String] = [String]()
    
    for number in B{
        if number.contains(P){
            foundMatch.append(A[indexPointer])
        }
        indexPointer+=1
    }
